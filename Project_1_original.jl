...
begin
	# Cell 1 – Load packages for UI, ODE solving, and plotting
	using PlutoUI
	using DifferentialEquations
	using Plots
	
	# Simple default plotting style
	default(linewidth = 2, legend = :right, size = (800, 450))
end
...
# ╔═╡ 3e351877-2f47-4d7b-bd53-06eb90e3ac64
begin
	begin
		# Cell 2 – Geometry and physical constants of the spinning frame + pendulum
		frame_height = 0.20      # h₁ in meters (frame vertical size)
		frame_width  = 0.10      # w₁ in meters (frame horizontal size)
		
		pendulum_length = 0.15   # L in meters
		pendulum_mass   = 0.10   # m in kilograms
		
		gravity_accel   = 9.81   # gravitational acceleration in m/s²
		
		# We will use the frame dimensions for drawings,
		# but they do not affect the pendulum dynamics directly.
		nothing
	end
end

# ╔═╡ 061498e6-b328-4a8e-91e9-870c1504b58a
begin
	# Cell 3 – Rotation rates of the frame and initial state of the pendulum
	rotation_rate_slow = 1.0      # Ω_slow in rad/s
	rotation_rate_fast = 8.0      # Ω_fast in rad/s
	
	initial_angle              = 0.20   # initial θ in radians
	initial_angular_velocity   = 0.0    # initial θ̇ in rad/s
	
	total_simulation_time = 20.0        # total time window in seconds
	time_span = (0.0, total_simulation_time)
	
	initial_state = [initial_angle, initial_angular_velocity]
end

# ╔═╡ b543849e-52fe-4f92-87df-78a8ca431109
# Cell 4 – Explanation of the Lagrange formulation (Markdown)
md"""
## Lagrange formulation for the spinning pendulum

We choose the generalized coordinate to be the **pendulum angle** \( \theta(t) \)
measured from the vertical direction (downwards).

In the rotating frame, the pendulum mass of length \( L \) has coordinates

\[
x'(t) = L \sin\theta(t), \qquad z'(t) = -L \cos\theta(t).
\]

The frame rotates around the global \( z \)-axis with constant angular speed \( \Omega \),
...
From these we compute the velocity and kinetic energy

\[
T = \tfrac{1}{2} m \big(\dot x^2 + \dot y^2 + \dot z^2\big)
  = \tfrac{1}{2} m L^2 \Big( \dot\theta^2 + \Omega^2 \sin^2\theta \Big).
\]

Taking the zero of potential energy at \( z = 0 \) gives
...
L = T - V
  ...
  + \tfrac{1}{2} m L^2 \Omega^2 \sin^2\theta
  + m g L \cos\theta.
\]
...
leads to the nonlinear equation of motion

\[
\ddot\theta
= \Big(\Omega^2 \cos\theta - \frac{g}{L}\Big)\sin\theta.
\]

We will use this equation directly in the numerical solver below.
"""

# ╔═╡ 66dac028-186e-4cac-a9aa-d5d217a47951
# Cell 5 – Time derivatives for the spinning pendulum state
function spinning_pendulum!(
    time_derivative_state,
    current_state,
    parameters,
    current_time,
)
    pendulum_angle            = current_state[1]
    pendulum_angular_velocity = current_state[2]

    pendulum_length = parameters.pendulum_length
    gravity_accel   = parameters.gravity_accel
    rotation_rate   = parameters.rotation_rate

    angle_acceleration =
        (rotation_rate^2 * cos(pendulum_angle) - gravity_accel / pendulum_length) *
        sin(pendulum_angle)

    time_derivative_state[1] = pendulum_angular_velocity
    time_derivative_state[2] = angle_acceleration
end
#cell5

# ╔═╡ f6975b68-9739-4720-b56c-2e67ff96609b
# Cell 6 – Run a simulation for a given rotation rate Ω
function simulate_spinning_pendulum(rotation_rate;
    ...
    simulation_time_span = (0.0, total_time)
    ...
    solve(problem, Tsit5(); reltol = 1e-9, abstol = 1e-9)
end
#cell6

# ╔═╡ f09673cd-f248-49a0-b008-49feda4246b1
begin
	# Cell 7 – Generate solutions for slow and fast frame rotation
	solution_slow = simulate_spinning_pendulum(rotation_rate_slow)
	solution_fast = simulate_spinning_pendulum(rotation_rate_fast)
	
	solution_slow, solution_fast
end

# ╔═╡ bdc049dc-ef2e-4335-9892-1da620512663
begin
	# Cell 8 – Plot the pendulum angle θ(t) for slow and fast rotation
	pendulum_angle_slow = [state[1] for state in solution_slow.u]
	pendulum_angle_fast = [state[1] for state in solution_fast.u]
	
	plot(
	    solution_slow.t,
	    pendulum_angle_slow;
	    label = "θ(t), Ω = $(rotation_rate_slow) rad/s",
	    xlabel = "time (s)",
	...
	plot!(
	    solution_fast.t,
	    pendulum_angle_fast;
	    label = "θ(t), Ω = $(rotation_rate_fast) rad/s",
	)
end

# ╔═╡ ecfa6ea2-2017-44b9-8116-d0bc64b56045
# Cell 9 – Helper: compute (x, y, z) of the mass from θ(t) and Ω
function pendulum_cartesian_position(pendulum_angle, time, rotation_rate)
    radial_distance = pendulum_length * sin(pendulum_angle)
    height_z        = -pendulum_length * cos(pendulum_angle)

    position_x = radial_distance * cos(rotation_rate * time)
    position_y = radial_distance * sin(rotation_rate * time)
    position_z = height_z
...
# ╔═╡ dcf9f148-f0b4-4f3d-b388-fdea6f2b96be
begin
	# Cell 10 - Choose slow or fast rotation for visualization
	md"**Use fast rotation (Ω_fast)**"
	
	@bind use_fast_rotation CheckBox(false)
end

# ╔═╡ 2dc4b255-277b-4623-8ace-35db24fb40c4
begin
	# Cell 11 - 3D animation of the pendulum tip as the frame spins
	selected_solution =
	    use_fast_rotation ? solution_fast : solution_slow
	selected_rotation_rate =
	    use_fast_rotation ? rotation_rate_fast : rotation_rate_slow
	
	animation_frames = @animate for (index, time_value) in
	    enumerate(selected_solution.t)
	    pendulum_angle = selected_solution.u[index][1]
	
	    position_x, position_y, position_z =
	        pendulum_cartesian_position(
	            pendulum_angle,
	            time_value,
	            selected_rotation_rate,
	        )
	    ...
	    scatter3d(
	        ...
	        ylim = (-pendulum_length, pendulum_length),
	        zlim = (-pendulum_length, pendulum_length),
	        xlabel = "x (m)",
	        ylabel = "y (m)",
	        zlabel = "z (m)",
	        title = "Spinning pendulum (Ω = $(selected_rotation_rate) rad/s)",
	    )
	    scatter3d!(
	        [position_x], [position_y], [position_z];
	        markersize = 8,
